...
public class WJ_Users {

    private double oppacity = 0;
    private double screenOppacity = 0;

    public WJ_Users(Stage primaryStage)
    {
        try{
            int userNumber = 0;

            GridPane grid = GridSettings("wifiJam", new int[]{20, 20, 20, 20, 20}, new int[]{16,16,16,16,16,16});
            ...
            for(int col=0;col<5;++col)
            {
                for(int row=1;row<6;++row)
                {
                    ...
                    {
                        ImageButton button = new ImageButton("WJ_button_User", primaryStage,col, row);
        ...
        {
            new Error(primaryStage,"usagers WifiJam");
        }
    }
